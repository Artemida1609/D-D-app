...
@Tag(name = "Equipment category", description = "Equipment category controller")
@RestController
...
public class EquipmentCategoryController {
    ...
    public EquipmentDto getByIndex(@PathVariable String index) {
        return equipmentService.getByOriginalIndex(index);
    }

    @GetMapping("/{category}")
    @Operation(summary = "get all",
            description = "get all elements of equipment category by category")
    public Page<EquipmentCategoryDto> getAllByCategory(
            @PathVariable String category,
            @ParameterObject
            @PageableDefault(sort = "name", direction = Sort.Direction.ASC) Pageable pageable
    ) {
        return equipmentCategoryService.getAllByCategory(category, pageable);
    }
}
