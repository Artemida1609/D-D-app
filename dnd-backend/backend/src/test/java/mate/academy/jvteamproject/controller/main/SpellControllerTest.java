...
public class SpellControllerTest extends TestContainersConfig {
    @Autowired
    ...
    private Spell expected;

    @BeforeEach
    void initData() {
        repository.deleteAll();

        expected = createSpell("spell", "Spell");
    ...
    void getByIndex_success() throws Exception {
        MvcResult mvcResult = mockMvc
                .perform(get("/api/spells/{index}", "spell")
                        .contentType(MediaType.APPLICATION_JSON)
                )
                .andExpect(status().isOk())
                .andReturn();
        SpellDto actual = objectMapper.readValue(mvcResult.getResponse().getContentAsString(),
                SpellDto.class);
    ...
    void getAll_success() throws Exception {
        ...
        SpellDto[] actual = objectMapper.readValue(root.get("content").toString(),
                SpellDto[].class);
